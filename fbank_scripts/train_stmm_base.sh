lang=$1
exp=mustc_en${lang}_stmm_self_learning_fbank_base
fairseq-train data/mustc/en-${lang} \
  --config-yaml config.yaml --train-subset train --valid-subset dev \
  --save-dir checkpoints/${exp} --num-workers 4 --max-tokens 20000 --max-update 100000 \
  ...
  --warmup-updates 10000 --clip-norm 10.0 --seed 1 --update-freq 4 \
  --patience 10 \
  --max-epoch 25 \
  --load-pretrained-asr-encoder-from checkpoints/mustc_en${lang}_asr_base.pt \
  --load-pretrained-mt-encoder-decoder-from checkpoints/mustc_en${lang}_mt_base.pt \
